...
async def deployer(state: VibeDeployState, config=None) -> dict:
    ...
    deploy_blocker = _get_deploy_blocker(frontend_code, backend_code, blueprint)
    if deploy_blocker:
        logger.error("[DEPLOYER] Generated code incomplete; skipping deployment: %s", deploy_blocker)
        await _emit_step_error("git_push", f"Skipping deployment: {deploy_blocker}", config=config, error=deploy_blocker)
        return {
            "deploy_result": {
                "app_id": "",
                "live_url": "",
...
def _get_deploy_blocker(frontend_code: dict, backend_code: dict, blueprint: dict | None) -> str | None:
