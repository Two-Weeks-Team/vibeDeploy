...
@pytest.mark.asyncio
async def test_generate_og_image_success(generator):
...
@pytest.mark.asyncio
async def test_generate_logo_success(generator):
...
@pytest.mark.asyncio
async def test_generate_og_image_http_error_returns_none(generator):
    mock_response = MagicMock()
...
@pytest.mark.asyncio
async def test_generate_logo_timeout_returns_none(generator):
    with patch("httpx.AsyncClient") as mock_client_cls:
        mock_client = AsyncMock()
        mock_client.__aenter__ = AsyncMock(return_value=mock_client)
        mock_client.__aexit__ = AsyncMock(return_value=False)
...
@pytest.mark.asyncio
async def test_generate_unexpected_exception_returns_none(generator):
    with patch("httpx.AsyncClient") as mock_client_cls:
        mock_client = AsyncMock()
        mock_client.__aenter__ = AsyncMock(return_value=mock_client)
        mock_client.__aexit__ = AsyncMock(return_value=False)
...
@pytest.mark.asyncio
async def test_og_image_prompt_contains_app_name_and_description(generator):
    fake_bytes = b"img"
    mock_response = MagicMock()
    mock_response.json.return_value = _make_b64_response(fake_bytes)
    mock_response.raise_for_status = MagicMock()
...
@pytest.mark.asyncio
async def test_logo_prompt_contains_app_name(generator):
    fake_bytes = b"logo"
    mock_response = MagicMock()
    mock_response.json.return_value = _make_b64_response(fake_bytes)
    mock_response.raise_for_status = MagicMock()
...
@pytest.mark.asyncio
async def test_request_uses_bearer_auth(generator):
